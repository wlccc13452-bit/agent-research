"""
# 生成新的拒绝方向并应用消融：
python ablate_glm.py --generate_refusal_dir --ablation_strength 5.0

# 加载之前的拒绝方向并应用消融：
python ablate_glm.py --ablation_strength 4.25

# 指定自定义缓存目录：
python ablate_glm.py --ablation_cache_dir "./custom_cache" --ablation_strength 3.5
"""
...
from save_load_refusal_dir import save_ablation_data, quick_load_latest_ablation_data
...
def load_prompts_from_txt(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            prompts = f.readlines()
        # 去除每行的换行符并格式化
        return [[{"role": "user", "content": prompt.strip()}] for prompt in prompts if prompt.strip()]
    except Exception as e:
        print(f"加载提示词文件失败: {e}")
        return []

def get_harmful_instructions():
    return load_prompts_from_txt('../data/harmful_prompts.txt'), []

def get_harmless_instructions():
    return load_prompts_from_txt('../data/harmless_prompts.txt'), []
...
# 计算拒绝生成
def refusal_generate(model, tokenized_input, bar=None):
    if bar is not None:
        bar.update(n=1)
    # 获取模型的正确设备
...
def analyze_layer_differences(model, harmful_outputs, harmless_outputs, harmful_last_positions, harmless_last_positions):
    """
    分析模型各层的差异，返回按不同指标排序的层信息。
    
    参数:
        model: 模型对象
        harmful_outputs: 有害指令的模型输出
        harmless_outputs: 无害指令的模型输出
        harmful_last_positions: 有害指令的最后token位置
        harmless_last_positions: 无害指令的最后token位置
...
def generate_refusal_dir(model):
    # 加载数据集
    ...
    harmful_outputs = [refusal_generate(model, toks, bar=bar) for toks in harmful_toks]
    harmless_outputs = [refusal_generate(model, toks, bar=bar) for toks in harmless_toks]
    bar.close()
    ...
    # 调用 analyze_layer_differences 函数
    analysis_results = analyze_layer_differences(
        model=model,
        harmful_outputs=harmful_outputs,
        harmless_outputs=harmless_outputs,
        harmful_last_positions=harmful_last_positions,
        harmless_last_positions=harmless_last_positions
    )
    
    # 打印分析结果
    print("\n=== 层差异分析结果 ===")
    print("按差异向量长度排序:")
    for i, info in enumerate(analysis_results["sorted_by_magnitude"]):
        print(f"  排名 {i+1}: 第{info['layer']}层, 差异长度={info['diff_magnitude']:.4f}")
    
    print("\n按平均绝对值排序:")
    for i, info in enumerate(analysis_results["sorted_by_mean_abs"]):
        print(f"  排名 {i+1}: 第{info['layer']}层, 平均绝对值={info['diff_mean_abs']:.6f}")
    
    print("\n按余弦相似度差异排序:")
    for i, info in enumerate(analysis_results["sorted_by_cosine"]):
        print(f"  排名 {i+1}: 第{info['layer']}层, 余弦差异={info['cosine_diff']:.4f}")
    
    print("\n按综合评分排序:")
    for i, info in enumerate(analysis_results["sorted_by_combined"]):
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="模型权重消融脚本")
    parser.add_argument(
        "--generate_refusal_dir", 
        action="store_true", 
        help="是否生成新的拒绝方向。如果未指定，则加载之前的最新结果"
    )
    parser.add_argument(
        "--ablation_strength", 
        type=float, 
    ...
    # 根据参数决定生成拒绝方向还是加载之前的结果
    if args.generate_refusal_dir:
        print("生成新的拒绝方向...")
        selected_layers, selected_refusal_directions = generate_refusal_dir(model)
        save_ablation_data(selected_layers, selected_refusal_directions, MODEL_ID, save_dir=args.ablation_cache_dir)
        print(f"✅ 拒绝方向已保存到缓存目录: {args.ablation_cache_dir}")
        print("请使用以下命令加载保存的数据并执行模型消融操作：")
        print(f"python ablate_glm.py --ablation_cache_dir {args.ablation_cache_dir} --ablation_strength {args.ablation_strength}")
        return  # 退出程序
    else:
        print("加载之前计算的最新拒绝方向...")
        selected_layers, selected_refusal_directions, metadata = quick_load_latest_ablation_data(
            MODEL_ID, search_dir=args.ablation_cache_dir
        )

    # 执行消融
    print(f"应用权重消融，消融强度为: {args.ablation_strength}")
    weight_manager = RefusalAblationWeightManager(model)
    weight_manager.apply_multiple_layers_ablation(
        selected_layers, selected_refusal_directions, ablation_strength=args.ablation_strength
    )

    # 标记模型为已修改
    model._weight_manager_modified = True

    # 保存修改后的模型
    save_modified_model_quantized_aware(
        model, 
        tokenizer, 
        src_dir=MODEL_ID, 
        out_dir=f'{MODEL_OUT_DIR}-{args.ablation_strength}'
    )
